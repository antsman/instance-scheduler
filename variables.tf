variable "schedule_expression" {
  default = "cron(5 * * * ? *)"
  description = "cloudwatch event rule scheule expression that specifies when the scheduler runs. Default is 5 minuts past the hour. for debugging use 'rate(5 minutes)'. See https://docs.aws.amazon.com/AmazonCloudWatch/latest/events/ScheduledEvents.html"
}

variable "tag" {
  default = "schedule"
  description = "tag name used on the EC2 or RDS instance to contain the schedule json string for the instance."
}
